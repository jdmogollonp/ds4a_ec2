...
# Connection to postgreSQL in AWS EC2
my_user = 'postgres'
my_passwd = 'postgres'


engine = create_engine('postgresql://'+my_user+':'+my_passwd+'@database-3.cdx68roruldf.us-east-1.rds.amazonaws.com/test')
...
#Callback for date updating selecting the leverage
@app.callback([dash.dependencies.Output('date-range', 'start_date'),
              dash.dependencies.Output('date-range', 'end_date')],
           [dash.dependencies.Input('exchange-select', 'value'),]             )
def update_startdate (exchange):
   dff_date = df[df['Exchange'] == exchange] 
